local stub = require("luassert.stub")
local match = require("luassert.match")
local mock = require("luassert.mock")
local storage = require("manuscript.storage")
local utils = require("manuscript.utils")

describe("storage testing", function()
  describe("load_last_draft", function()
    -- Create stubs for the vim functions
    local expand_stub
    local readable_stub
    local readfile_stub
    local utils_stub

    before_each(function()
      expand_stub = stub(vim.fn, "expand")
      readable_stub = stub(vim.fn, "filereadable")
      readfile_stub = stub(vim.fn, "readfile")
      utils_stub = stub(utils, "get_current_directory")
    end)

    after_each(function()
      expand_stub:revert()
      readable_stub:revert()
      readfile_stub:revert()
      utils_stub:revert()
    end)

    it("returns file content when the draft exists", function()
      -- Arrange: Setup the "fake" environment
      expand_stub.returns("/abs/path/vault")
      utils_stub.returns("my-project")
      readable_stub.returns(1) -- 1 means 'true' in VimScript
      readfile_stub.returns({ "line 1", "line 2" })

      -- Act: Call your function
      local result = storage.load_last_draft("~/vault")

      -- Assert: Verify the logic
      assert.are.same({ "line 1", "line 2" }, result)
      assert.stub(readable_stub).was_called_with("/abs/path/vault/my-project-draft.md")
    end)

    it("returns nil when the draft does not exist", function()
      -- Arrange
      expand_stub.returns("/abs/path/vault")
      utils_stub.returns("empty-project")
      readable_stub.returns(0) -- File not found

      -- Act
      local result = storage.load_last_draft("~/vault")

      -- Assert
      assert.is_nil(result)
      assert.stub(readfile_stub).was_not_called()
    end)
  end)

  describe("save_file", function()
    local expand_stub
    local isdirectory_stub
    local makedir_stub
    local utils_stub
    local writefile_stub
    local getlines_stub
    local setoption_stub
    local notify_stub

    before_each(function()
      expand_stub = stub(vim.fn, "expand")
      isdirectory_stub = stub(vim.fn, "isdirectory")
      makedir_stub = stub(vim.fn, "mkdir")
      utils_stub = stub(utils, "get_current_directory")
      writefile_stub = stub(vim.fn, "writefile")
      getlines_stub = stub(vim.api, "nvim_buf_get_lines")
      setoption_stub = stub(vim.api, "nvim_buf_set_option")
      notify_stub = stub(vim, "notify")
    end)

    after_each(function()
      expand_stub:revert()
      isdirectory_stub:revert()
      makedir_stub:revert()
      utils_stub:revert()
      writefile_stub:revert()
      getlines_stub:revert()
      setoption_stub:revert()
      notify_stub:revert()
    end)

    it("creates the directory if it is missing and saves the file", function()
      local test_buf = 42
      local vault_path = "~/my-vault"
      expand_stub.returns("/abs/my-vault")
      utils_stub.returns("chapter1")
      isdirectory_stub.returns(0) -- Directory does NOT exist
      getlines_stub.returns({ "content line 1" })

      storage.save_file(test_buf, vault_path)

      assert.stub(makedir_stub).was_called_with("/abs/my-vault", "p")
      assert.stub(writefile_stub).was_called_with({ "content line 1" }, "/abs/my-vault/chapter1-draft.md")
      assert.stub(setoption_stub).was_called_with(test_buf, "modified", false)
    end)

    it("handles write errors gracefully using pcall", function()
      expand_stub.returns("/abs/vault")
      utils_stub.returns("fail-file")
      isdirectory_stub.returns(1)

      writefile_stub.invokes(function() error("Disk Full!") end)
      storage.save_file(101, "/abs/vault")

      assert.stub(setoption_stub).was_not_called()
    end)
  end)
end)
